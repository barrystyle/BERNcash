...
unsigned int nStakeMinAge = 60 * 60 * 24;       // 24 hour
unsigned int nStakeMaxAge = 60 * 60 * 35040;    // 4 years
unsigned int nStakeTargetSpacing = 60;          // 1 minute block spacing

int64 nChainStartTime = 1455848035; // launch 1456203600
...
const string strMessageMagic = "BERN Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1) {
    nSubsidy = 10000000 * COIN; // BERN instamine. 5% of 200 mil = 10 mil
    } else if(nHeight >= 2 && nHeight <= 720000) { // BERN target 250 days for first phase. Roughly ending a week prior to election from launch.
    nSubsidy = 250 * COIN; //  90 mil plus 10 mil insta = half the coins in circulation.
    } else if(nHeight >= 720001 && nHeight <= 3875760) { // 3 additional years
    nSubsidy = 25 * COIN; // 25 = 39,447,000. 60,553,000 remain
    } else if(nHeight >= 3875761 && nHeight <= 4927680) { // 1 additional year
    nSubsidy = 100 * COIN; // 52,596,000. 7,957,000 remain
    } else if(nHeight >= 4927681 && nHeight <= 6519080) { // we want this 795,700 times. 1,591,400 blocks
    nSubsidy = 10 * COIN;
    }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_BERN_PROOF_OF_STAKE;
    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;  // BERN no / COIN so PoS is correct
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BERN
    bool fStrictPayToScriptHash = true; // Always active in BERN
    ...
    auto prevHash = pindex->pprev->GetBlockHeader().GetHash();
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BERN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "BERNcash - the peoples coin! 23/2/2016";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x37df7239291d8195fb2701235c2a6d76f62f8412c38e070a7f962c887da0d711"));
...
unsigned char pchMessageStart[4] = { 0x09, 0x11, 0x16, 0x20 };
